...
# shazamio>=0.8 requires numpy 2.x, so we can't pin numpy<2. numpy 2.x wheels
# assume the x86-64-v2 CPU baseline (universal on cloud/VPS hosts from ~2013+
# and irrelevant on ARM); only truly ancient x86 hardware lacks it. Debian
# trixie so glibc matches the Olaf binary built above.
...
RUN pip install --no-cache-dir \
      ...
      "PyYAML>=6.0" "numpy>=2.2.2" "soundfile>=0.12" "shazamio>=0.8"
