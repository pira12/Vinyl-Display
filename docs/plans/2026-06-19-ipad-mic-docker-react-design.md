...
The user already runs **Traefik** with split-horizon DNS for `example.com` on the
LAN and deploys containers with **Dokploy**, so TLS and routing are solved
upstream. Target hostname: `vinyl.example.com`.
...
```
iPad (Safari, https://vinyl.example.com)        Pi container (Traefik-fronted)
...
- Dokploy compose with Traefik labels: `Host(\`vinyl.example.com\`)`,
